# Claude Multi-Agent Console &nbsp;·&nbsp; v0.8.0
...
| OS | 파일 |
| --- | --- |
| Windows | `ClaudeMultiAgentConsole-0.8.0.exe` |
| macOS (Apple Silicon) | `ClaudeMultiAgentConsole-0.8.0.dmg` |
| Linux | `ClaudeMultiAgentConsole-0.8.0.AppImage` |
...
```
[로컬]                                          [원격 Linux 서버]
  ClaudeMultiAgentConsole-0.8.0.exe             ~/Claude_Multi_Agent_Console/
...
```
C:\Apps\AgentConsole\
├── ClaudeMultiAgentConsole-0.8.0.exe
├── config.json     ← 자동 생성
...
npm install
npm run dist:win    # → dist-installers/ClaudeMultiAgentConsole-0.8.0.exe
```

Linux: `npm run dist:linux` → `.AppImage`. Mac (Apple Silicon): `npm run dist:mac` → `.dmg` (Mac 머신 필요).

> v0.8.0부터 `start.sh` / `start.bat` / `npm start` 같은 수동 실행 경로는 더 이상 제공하지 않습니다. 배포는 `.exe` / `.dmg` / `.AppImage` 한 가지 경로로만 진행됩니다.
